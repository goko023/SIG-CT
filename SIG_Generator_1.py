...
class LoreEditor(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(hdr, text="Lore", font=("Consolas", 12, "bold"),
                     text_color="#b9bbbe").pack(side="left")
        ...
        self.container = ctk.CTkFrame(self, fg_color="#2b2d31", corner_radius=4)
        self.container.pack(fill="x", pady=(4, 0))

    def add_row(self, text="", color="White"):
        row_frame = ctk.CTkFrame(self.container, fg_color="#313338", corner_radius=4)
...
class UsageEditor(ctk.CTkFrame):
    ...
    def add_block(self, trigger="right", cancel=True, consume=0, cmds=None, at="any"):
        cmds = cmds or []
        block_frame = ctk.CTkFrame(self.container, fg_color="#313338", corner_radius=4)
...
class ItemEditor(ctk.CTkFrame):
    def __init__(self, parent, on_change):
        super().__init__(parent, fg_color="#1e1f22")
        # Inner scrollable — single scrollbar, no duplicates
        self._scroll = ctk.CTkScrollableFrame(self, fg_color="#1e1f22",
                                               scrollbar_fg_color="#2b2d31",
                                               scrollbar_button_color="#5865f2",
                                               scrollbar_button_hover_color="#4752c4")
    ...
    def _build(self):
        ...
        flags_frame = ctk.CTkFrame(self._scroll, fg_color="#2b2d31", corner_radius=4)
    ...
    def _section(self, title):
        ctk.CTkLabel(self._scroll, text=title,
                     ...
                     text_color="#b9bbbe").pack(anchor="w", padx=14, pady=(10, 2))
    ...
    def _sep(self):
        ctk.CTkFrame(self._scroll, fg_color="#3f4147", height=1).pack(fill="x", pady=6)
...
class App(ctk.CTk):
    ...
    def _build_header(self):
        hdr = ctk.CTkFrame(self, fg_color="#1e1f22", height=52, corner_radius=0)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)

        ctk.CTkLabel(hdr, text="SIG Generator",
    ...
    def _build_welcome(self):
        self.welcome_frame = ctk.CTkFrame(self, fg_color="#111214")
        ...
        try:
            ...
            img_label.configure(text="[ eh.png not found ]", font=("Consolas", 12), text_color="#ed4245")

        # Right: title + buttons
        right = ctk.CTkFrame(inner, fg_color="transparent")
        right.pack(side="left")

        ctk.CTkLabel(right, text="SIG Generator",
                     font=("Consolas", 26, "bold"),
                     text_color="#ffffff").pack(anchor="w")
        ctk.CTkLabel(right, text="Visual configurator for SimpleItemGenerator",
                     font=("Consolas", 12),
                     text_color="#334455").pack(anchor="w", pady=(4, 32))
        ...
        ctk.CTkButton(right, text="Open Project",
                      height=44, width=280,
                      font=("Consolas", 13),
                      fg_color="#111122", hover_color=BTN,
                      corner_radius=4,
                      border_width=1, border_color="#2a3a5a",
                      command=self._load_project).pack()

        ctk.CTkLabel(right, text="v2.0  --  SimpleItemGenerator Config Tool",
                     font=("Consolas", 10),
                     text_color="#222233").pack(anchor="w", pady=(28, 0))
    ...
    def _build_main_layout(self):
        ...
        self.left_panel = ctk.CTkFrame(self.main_frame,
                                       fg_color="#1e1f22", corner_radius=4)
        self.left_panel.pack(side="left", fill="both", expand=True, padx=(0, 6))

        self.right_panel = ctk.CTkFrame(self.main_frame,
                                        fg_color="#1e1f22", corner_radius=4, width=400)
    ...
    def _build_left_panel(self):
        ...
        list_hdr = ctk.CTkFrame(self.left_panel, fg_color="#2b2d31",
        ...
        self.item_list_frame = ctk.CTkScrollableFrame(
            self.left_panel, height=140, fg_color="#1e1f22",
            scrollbar_fg_color="#2b2d31",
            scrollbar_button_color="#5865f2",
            scrollbar_button_hover_color="#4752c4",
        )
        ...
        ctk.CTkFrame(self.left_panel, fg_color="#3f4147", height=1).pack(fill="x")

        # --- Alt: Editor alanı (tüm kalan alan) ---
        self.editor_container = ctk.CTkFrame(self.left_panel, fg_color="#1e1f22")
        self.editor_container.pack(fill="both", expand=True)

        self.editor_label = ctk.CTkLabel(
            self.editor_container,
            text="Select an item or add a new one",
            font=("Consolas", 12), text_color="#5c6370"
        )
        self.editor_label.pack(pady=20)

    def _build_right_panel(self):
        top = ctk.CTkFrame(self.right_panel, fg_color="#2b2d31",
                           height=40, corner_radius=0)
        top.pack(fill="x")
    ...
    def _select_item(self, idx: int):
        ...
        if hasattr(self, "editor_label") and self.editor_label.winfo_exists():
    ...
    def _rebuild_item_list(self):
        ...
        if not self.items:
            ctk.CTkLabel(self.item_list_frame,
                         ...
                         font=("Consolas", 11), text_color="#5c6370"
                         ).pack(pady=10)
            return
        ...
        for i, item in enumerate(self.items):
            ...
            row = ctk.CTkFrame(self.item_list_frame,
                               fg_color="#404375" if is_sel else "#2b2d31",
                               corner_radius=4)
            ...
            ctk.CTkLabel(row,
                         ...
                         text_color="#ffffff" if is_sel else "#8d9096",
                         anchor="w").pack(side="left", padx=10, pady=6,
                                         fill="x", expand=True)

            ctk.CTkLabel(row, text=item.get("material", ""),
                         font=("Consolas", 10),
                         text_color="#5c6370").pack(side="left", padx=6)
